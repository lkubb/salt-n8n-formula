...
We will see a quick setup to configure the ``n8n`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``n8n`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``n8n`` formulas under ``/srv/salt/n8n/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/n8n/parameters/dns:domain/
    ln -s dns:domain /srv/salt/n8n/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/n8n/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/n8n/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``n8n`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``n8n`` formulas under ``/srv/salt/n8n/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/n8n/parameters/roles
...
We create a configuration for the role ``n8n/server`` in ``/srv/salt/n8n/parameters/roles/n8n/server.yaml``:
...
We create another configuration for the role ``n8n/client`` in ``/srv/salt/n8n/parameters/roles/n8n/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles n8n/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles n8n/client
...
.. code-block:: console
    ...
    mv /srv/salt/n8n/parameters/dns:domain/  '/srv/salt/n8n/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-n8n-as-json:
      test.nop:
        - name: {{ n8n | json }}
...
Here is an example based on `template-formula/n8n/config/file.sls`_:
    ...
    n8n-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='n8n-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``n8n`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ n8n['winner'] }}
...
.. _template-formula/n8n/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/n8n/config/file.sls
